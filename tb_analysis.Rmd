...
print(paste(
  "Filtering complete. Dataset reduced from", nrow(tb_global_2024_clean),
...
tb_global_2024_pivoted <- tb_global_2024_filtered %>%
  ...
  rename(
    country = dim_geo_name,
    iso3 = dim_geo_code,
    year = dim_time_year
  )

print(paste(
  "Pivoting complete. Dataset now has", nrow(tb_global_2024_pivoted), "rows and",
...
print(paste(
  "Years covered:", min(tb_global_2024_pivoted$dim_time_year, na.rm = TRUE),
...
if (nrow(timor_leste_check) > 0) {
  ...
  if (nrow(column_completeness) == 0) {
    tl_global_complete <- FALSE
  } else {
    tl_global_complete <- TRUE
  }
} else {
...
pwalk(
  list(
    key_vars_summary$variable, key_vars_summary$exists,
...
if (nrow(timor_filtered) > 0) {
  ...
  # Display results
  walk2(
    timor_vars_summary$variable, timor_vars_summary$non_na_count,
...
if (file.exists(enhanced_file)) {
  ...
  # Display using walk2
  walk2(
    sample_descriptions$col_name, sample_descriptions$description,
...
variable_summary %>%
  ...
  group_walk(~ {
    cat("\n", toupper(.y$category), ":\n")
    walk2(
      .x$variable, .x$description_short,
      ~ cat("  -", .x, ":", .y, "\n")
    )
  })
...
# Display using walk2
walk2(
  category_counts$category, category_counts$vars_total,
  ~ cat("  -", .x, ":", .y, "variables\n")
)
```
...
# Display using walk2
walk2(
  category_summary$category, category_summary$n_vars,
  ~ cat("  *", .x, ":", .y, "variables\n")
)
...
if (length(missing_vars) > 0) {
  ...
  walk2(
    missing_vars_summary$variable, missing_vars_summary$category,
    ~ cat("  *", .x, "(", .y, ")\n")
  )
} else {
...
regional_cfr_plot_clean <- country_cfr_data %>%
  ...
  geom_line(
    data = . %>% filter(country_type == "Other Countries"),
    aes(group = country),
    alpha = 0.15, color = "gray50", size = 0.3
  ) +
  ...
  geom_line(
    data = . %>% filter(country_type == "Timor-Leste"),
    aes(group = country),
    color = "#E31A1C", size = 0.3, alpha = 0.9
  ) +
...
# Simple one-line export
write_csv(
  correlation_analyses$timor_leste_focus %>% as.data.frame() %>% rownames_to_column("variable"),
...
Method D: Using table_regression
